...
public class ArrayList<T> implements List<T>, Serializable {
  ...
  // start from 0 like normal array
  @Override
  ...
  // replace all entry equals to oldElem
  public void replaceAll(T oldElem, T newElem) {
    int i = 0;
    while ((i = indexOf(oldElem)) != -1) {
      set(i, newElem);
    }
  }

  // check if list is empty(not the actual array)
  @Override
  ...
  // add the object "t" at the given index
  // will move all elements backward by 1 starting from the index
  @Override
  public void add(int index, T element) {
    rangeCheckForAdd(index);
    final int s;
    Object[] elementData;
    if ((s = size) == (elementData = this.arr).length)
      elementData = grow();
  ...
  // remove all element in the array and set list size to 0
  @Override
  public void clear() {
    final Object[] es = arr;
  ...
  // only return the index of first object found if there are duplicates
  ...
  // return the index of the object
  // only return the index of last object found if there are duplicates
  // return -1 if not found
  @Override
  ...
  // check if this list is equals to the object given
  // return true only when the given object is a list, list has the same length,
  ...
  public boolean equals(Object obj) {
    ...
    boolean equal = (obj.getClass() == ArrayList.class)
        ? equalsArrayList((ArrayList<?>) obj)
        : equalsRange((List<?>) obj, 0, size);
    return equal;
  }

  // return the iterator for foreach loop or in case needed
  ...
  // return the iterator for foreach loop or in case needed
  // added in for specification purpose, default just use iterator()
  @Override
  ...
  private class Itr implements Iterator<T> {
    ...
    public T next() {
      ...
      Object[] elementData = ArrayList.this.arr;
      cursor = i + 1;
